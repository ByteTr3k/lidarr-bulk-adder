# lidarr-bulk-adder/app.py
 ...
 APP_VERSION = "0.1.0"  # <-- Set your desired version number here
 # ---
 

 CONFIG_FILE = 'config.json'  # <-- Keeping your preferred config file location
 ...
 def index():
  ...
  # *** MODIFIED: Pass app_version to the template ***
  return render_template('index.html', config=config, app_version=APP_VERSION)
 

 @app.route('/add', methods=['POST'])
 def add_artists_route():
  """Handles adding artists from text input or file upload and returns JSON progress."""
  config = load_config()
  results = []
  artists_input = ""
  ...
  if file.filename != '':
   try:
    ...
    return jsonify({"status": "error", "message": f"Error reading uploaded file: {e}"}), 400  # Bad Request
  ...
  processed_count += 1
  print(f"Processed {processed_count}/{total_artists}")
  time.sleep(0.5)  # Add a small delay to simulate processing time
  ...
  return jsonify({
  "status": "complete",
  "message": f"Processing complete: Added: {summary['added']}, Already Existed: {summary['already_exists']}, Not Found: {summary['not_found']}, Errors: {summary['error']} (Total: {summary['total']})",
  "summary": summary,
 ...
 def settings():
  ...
  # --- POST logic remains the same as your version ---
  ...
  if not new_config['LIDARR_URL'] or not new_config['API_KEY'] or not new_config['ROOT_FOLDER_PATH']:
   ...
   # *** MODIFIED: Pass app_version when re-rendering template on error ***
  ...
  # *** MODIFIED: Pass app_version to the template for GET request ***
